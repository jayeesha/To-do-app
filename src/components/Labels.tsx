...
const Labels: React.FC<{
  ...
  setSelectedControl: (value: React.SetStateAction<number>) => void;
  resetTimerValues: () => void;
  setPomodoro: Dispatch<SetStateAction<PomodoroData>>;
  setIsSettingsOn: (value: React.SetStateAction<boolean>) => void;
}> = ({
  selectedControl,
  setSelectedControl,
  resetTimerValues,
  ...
  const handleSelectedControl = (index: number): void => {
  ...
  return (
    <div className="flex">
      ...
      <button onClick={() => setIsSettingsOn(true)} className="ml-auto">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          height="1em"
          viewBox="0 0 512 512"
          strokeWidth={1.5}
          className="w-6 h-6 m-6 fill-violet-600 dark:fill-violet-800"
        >
          <path
            fill="#7C3AE"
